...
def encode_img_data(img):
    ...
    print("\nEncoded the data successfully in the Image, and the image is successfully saved with the name ", nameoffile)
    ...
    for i in img:
        for pixel in i:
            ...
            for byte in total_bytes:
                ...
                if decoded_data[-5:] == "*^*^*":
                    print("\n\nThe Encoded data that was hidden in the Image was :--  ", decoded_data[:-5])
                    return

def img_steg():
    while True:
        h2_label = customtkinter.CTkLabel(app, text="IMAGE STEGANOGRAPHY OPERATIONS")
        h2_label.pack(padx=10, pady=10)

        enc_text_msg_button = customtkinter.CTkButton(app, text="Encode the Text message", width=140)
        enc_text_msg_button.pack(padx=20, pady=10)
        
        dec_text_msg_button = customtkinter.CTkButton(app, text="Decode the Text message", width=140)
